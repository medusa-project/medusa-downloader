...
class SqsPollerState
  ...
  class << self
    ...
    def mark_running!(pid: Process.pid)
      write_state(pid,
        'status' => 'running',
        'pid' => pid,
    ...
    def mark_heartbeat!(pid = Process.pid)
      state = read_state(pid)
      write_state(pid, 'last_heartbeat_at' => Time.now.utc.iso8601) if state['status'] == 'running'
    end
    ...
    def state_file(pid)
      File.join(STATE_DIR, "sqs_poller_state_#{pid}.json")
    end

    def all_worker_states
      FileUtils.mkdir_p(STATE_DIR)
      Dir.glob(File.join(STATE_DIR, 'sqs_poller_state_*.json')).filter_map do |file|
    ...
    def write_state(pid, updates)
      ...
      Rails.logger.error "Failed to write SQS poller state: #{e}"
    end
    ...
    def start_heartbeat_thread(pid)
      Thread.new do
        ...
        Rails.logger.error "SQS poller heartbeat thread error: #{e}"
      end
    end
  end
end
